...
* Run airflow initdb which generates a new config file `webserver_config.py` under the AIRFLOW_HOME.
* This file contains the default settings for working with RBAC
* You should see a new `Security` menu in the Airflow UI

###Enabling the plugin support under RBAC
* After installing the plguin you will see few dynamic permissions generated by RBAC
* Goto List roles menu item under `Security` menu
* Edit the Public role
* Add `menu access on Rest API Plugin` and `can secure check on Rest API` permissions
